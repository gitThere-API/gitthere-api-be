...
// app.use('/api', ensureAuth); //********Commented out to enable demopage*********** added individually to '/api/test', '/api/favorites', 

// and now every request that has a token in the Authorization header will have a `req.userId` property for us to see who's talking
app.get('/api/test', ensureAuth, (req, res) => {
  res.json({
...
app.get('/api/bird', async(req, res) => {
  try {
    const URL = 'https://mds.bird.co/gbfs/portland/free_bike_status.json';
...
app.get('/api/bolt', async(req, res) => {
  try {
    const URL = 'https://api-lb.micromobility.com/ptl/gbfs/en/free_bike_status.json';
...
app.get('/api/trimet/detail/:locid', async(req, res) => {
  try {
    const URL = `https://developer.trimet.org/ws/v2/arrivals?appID=${process.env.TRIMET_APP_ID}&locIDs=${req.params.locid}`;
